  ...
  <div>
    ...
    <section class="relative overflow-hidden">
      ...
      <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-24 sm:py-40 text-center">
        ...
        <template v-else>
          ...
          <h1 class="text-4xl sm:text-6xl font-extrabold tracking-tight text-gray-900 dark:text-white drop-shadow-lg font-serif">
    ...
    <section v-if="featured?.data?.length" class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
      <div class="flex items-center justify-between mb-8">
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Communautés à la une</h2>
    ...
    <section v-if="stats?.topModules?.length" class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-16">
      <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-8">Modules les plus représentés</h2>
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
        <NuxtLink
          v-for="mod in stats.topModules.slice(0, 10)"
          :key="mod.name"
          :to="`/communautes?modules=${encodeURIComponent(mod.name)}`"
          class="rounded-lg border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50 p-4 text-center hover:border-blue-500/50 hover:bg-gray-100 dark:hover:bg-gray-900 transition-colors"
        >
    ...
    <section class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-16">
      <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-8">En images</h2>
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        <div
          v-for="img in galleryImages"
          :key="img.src"
          class="group relative aspect-video overflow-hidden rounded-lg border border-gray-200 dark:border-gray-800"
        >
          <img
            :src="img.src"
            :alt="img.alt"
            ...
            loading="lazy"
          />
          <div class="absolute inset-0 bg-linear-to-t from-white/80 dark:from-gray-950/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
          <span class="absolute bottom-2 left-2 text-xs font-medium text-gray-900 dark:text-white opacity-0 group-hover:opacity-100 transition-opacity">
            {{ img.alt }}
          </span>
        </div>
      </div>
