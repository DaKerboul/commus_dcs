<template>
  <div>
    <h1 class="text-2xl font-bold text-white mb-6">Soumissions</h1>
    ...
    <div v-else class="space-y-3">
      <div
        ...
        :key="sub.id"
        class="rounded-lg border border-gray-800 bg-gray-900/50 p-5"
      >
        <div class="flex items-start justify-between gap-4">
          <div>
            <div class="flex items-center gap-2">
              <span class="font-semibold text-white">{{ sub.communityName }}</span>
              <UBadge
            ...
            <div class="text-sm text-gray-400 mt-1">Contact: {{ sub.contactName }}</div>
            ...
            <p v-if="sub.description" class="text-sm text-gray-400 mt-2">{{ sub.description }}</p>
