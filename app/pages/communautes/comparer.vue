<template>
  <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-8">
      <h1 class="text-3xl font-bold text-white">Comparer des communautés</h1>
      <p class="mt-2 text-gray-400">Sélectionnez 2 à 3 communautés pour les comparer côte à côte.</p>
    </div>
    ...
    <div v-if="comparedCommunities.length >= 2" class="overflow-x-auto">
      <table class="w-full border-collapse">
        <thead>
          <tr>
            <th class="text-left p-3 text-sm text-gray-400 font-medium border-b border-gray-800 w-48">Critère</th>
            <th
              v-for="c in comparedCommunities"
              :key="c.slug"
              class="p-3 text-center border-b border-gray-800"
            >
              <NuxtLink :to="`/communautes/${c.slug}`" class="text-white font-semibold hover:text-blue-400">
                {{ c.name }}
              </NuxtLink>
            </th>
          </tr>
        ...
        <tbody>
          <tr class="border-b border-gray-800/50">
            <td class="p-3 text-sm text-gray-400">Type</td>
            <td v-for="c in comparedCommunities" :key="c.slug" class="p-3 text-center text-sm text-white">
              {{ TYPE_LABELS[c.communityType] || c.communityType }}
            </td>
          </tr>
          <tr class="border-b border-gray-800/50">
            <td class="p-3 text-sm text-gray-400">Taille</td>
            <td v-for="c in comparedCommunities" :key="c.slug" class="p-3 text-center text-sm text-white">
              {{ c.sizeText || SIZE_LABELS[c.sizeCategory] }}
            </td>
          </tr>
          <tr class="border-b border-gray-800/50">
            <td class="p-3 text-sm text-gray-400">Recrutement</td>
          ...
          <tr class="border-b border-gray-800/50">
            <td class="p-3 text-sm text-gray-400">Événements</td>
            <td v-for="c in comparedCommunities" :key="c.slug" class="p-3 text-center text-sm text-white">
              {{ FREQUENCY_LABELS[c.eventFrequency] }}
            </td>
          </tr>
          <tr class="border-b border-gray-800/50">
            <td class="p-3 text-sm text-gray-400">Modules DCS</td>
            <td v-for="c in comparedCommunities" :key="c.slug" class="p-3 text-center">
          ...
          <tr class="border-b border-gray-800/50">
            <td class="p-3 text-sm text-gray-400">Expériences</td>
            <td v-for="c in comparedCommunities" :key="c.slug" class="p-3 text-center">
              <div class="flex flex-wrap gap-1 justify-center">
          ...
          <tr class="border-b border-gray-800/50">
            <td class="p-3 text-sm text-gray-400">Périodes</td>
            <td v-for="c in comparedCommunities" :key="c.slug" class="p-3 text-center text-sm text-white">
              {{ c.historicalPeriods?.map((p: string) => PERIOD_LABELS[p] || p).join(', ') || '—' }}
            </td>
          </tr>
          <tr class="border-b border-gray-800/50">
            <td class="p-3 text-sm text-gray-400">Discord</td>
            <td v-for="c in comparedCommunities" :key="c.slug" class="p-3 text-center">
    ...
    <div v-else class="text-center py-16">
      ...
      <p class="mt-4 text-gray-400">Sélectionnez au moins 2 communautés pour les comparer.</p>
    </div>
  </div>
</template>
